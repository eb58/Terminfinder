...
const slotId = (date, time) => `${date}T${time}`;
const formatSlot = slot => `${formatter.format(new Date(`${slot.date}T${slot.time}`))} ${slot.time}`;
...
const normalizeServerState = serverState => ({
  ...
  availability: serverState?.availability && typeof serverState.availability === "object"
    ...
    : {},
  slots: normalizeSlots(serverState?.slots),
});
...
const addSlot = async (date, time) => {
  if (!date || !time || state.slots.some(slot => slot.id === slotId(date, time))) return;
  state.slots = [...state.slots, { id: slotId(date, time), date, time, order: state.slots.length }];
  render();
  await save();
};
...
const updateSlot = async (oldId, date, time) => {
  ...
  if (!date || !time || oldId === newId) return;
  if (state.slots.some(slot => slot.id === newId)) return;
  state.slots = state.slots.map(slot => slot.id === oldId ? { ...slot, id: newId, date, time } : slot);
